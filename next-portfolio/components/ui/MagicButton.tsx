  ...
  return (
	<div>
		<button className="relative inline-flex h-12 overflow-hidden rounded-lg p-[1px] focus:outline-none" onClick={handleClick}>
			...
			<span className={`tracking-wider w-[200px] lg:tracking-widest sm:tracking-normal inline-flex h-full cursor-pointer items-center justify-center rounded-lg bg-slate-950 px-7 py-1 text-sm font-medium text-white backdrop-blur-3xl gap-2 ${otherClasses}`}>
			{position === 'right' && icon}
	  		{title}
			{(position === 'left' && icon)}
			</span>
